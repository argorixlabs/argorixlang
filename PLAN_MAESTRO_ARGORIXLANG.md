...
Última actualización de estado: 2026-09-23 sobre `main@21b80ca`. **ESP-009
(biblioteca estándar mínima) está HECHA** (PRs #47, #48 y #51, con CI verde
en `main`). Incluye:

- ocho módulos `.argx` (`result`, `bytes`, `text`, `vector`, `arena`,
  `ordered_map`, `path` y `json`);
...
stage0 en 49 muestras. **ESP-011 (parser en Argorix) está HECHA** (PR #54): el
parser y el AST en `compiler/` coinciden con el parser stage0 en 135 archivos.
**ESP-012 a ESP-016 están HECHAS** (PRs #55 a #61): el checker y el linker,
