//! ESP-018.D: the Argorix agent-language IR (`compiler/agent_ir.argx`)
//! against stage0's `IrProgram::from` (`crates/argorix_ir/src/ir.rs`).
//!
//! `tests/selfhost/agent/ir_files.argx` is compiled through the transitional
//! C backend and run with a package root holding every sample. For each it
//! writes the canonical dump of `spec/language/ir.md`: the IR as pretty
//! JSON, or the checker dump of a program that does not check. Each dump
//! must equal, byte for byte, what stage0 produces (`argorixc agent-ir`).
...
use argorix_module::agent_ir_dump;
use std::path::{Path, PathBuf};
use std::process::Command;
use std::{env, fs};

/// How many samples check and so lower to IR. Raise it when the corpus
...
#[cfg(unix)]
#[test]
fn the_argorix_agent_ir_matches_the_stage0_ir_when_cc_is_available() {
    ...
    let work = env::temp_dir().join(format!("argorix-agent-ir-{}", std::process::id()));
    ...
    let c_file = work.join("ir_files.c");
    let emit = Command::new(env!("CARGO_BIN_EXE_argorixc"))
        ...
        .arg("core-emit-c")
        .arg(root().join("tests/selfhost/agent/ir_files.argx"))
        .arg("--output")
        .arg(&c_file)
        .output()
        .unwrap();
    ...
    let executable = work.join("ir_files");
    ...
    for (index, (name, source)) in samples.iter().enumerate() {
        let expected = agent_ir_dump(source);
        if expected.starts_with('{') {
            lowered += 1;
        }
        let actual = fs::read(build.join(format!("dumps/{index}.ir"))).unwrap();
    ...
    eprintln!(
        "agent IR: {matched} match ({lowered} lowered), {} differ, of {}",
        failed.len(),
        samples.len()
    );
    assert!(
        failed.is_empty(),
        "the Argorix agent IR disagrees on {} samples:\n{}",
        failed.len(),
        failed
            .iter()
            .take(20)
