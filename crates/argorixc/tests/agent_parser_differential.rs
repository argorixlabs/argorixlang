//! ESP-018.B: the Argorix agent-language parser
//! (`compiler/agent_parser.argx`) against the stage0 one
//! (`argorix_parser::parser`).
//!
//! `tests/selfhost/agent/parse_files.argx` is compiled through the
...
//! syntax tree as serde_json writes it, or the first diagnostic. Each dump
//! must equal, byte for byte, what stage0 produces (`argorixc agent-ast`).
//!
...
use argorix_parser::parser::ast_dump;
...
const MATCHED_AT_LEAST: usize = 496;
...
#[cfg(unix)]
#[test]
fn the_argorix_agent_parser_matches_the_stage0_parser_when_cc_is_available() {
    ...
    let work = env::temp_dir().join(format!("argorix-agent-parser-{}", std::process::id()));
    ...
    let emit = Command::new(env!("CARGO_BIN_EXE_argorixc"))
        ...
        .arg("core-emit-c")
        .arg(root().join("tests/selfhost/agent/parse_files.argx"))
        .arg("--output")
        .arg(&c_file)
        .output()
        .unwrap();
    ...
    eprintln!(
        "agent parser: {matched} match, {unsupported} use declarations not ported yet, {} differ, of {}",
        failed.len(),
        samples.len()
    );
    assert!(
        failed.is_empty(),
        "the Argorix agent parser disagrees on {} samples:\n{}",
        failed.len(),
        failed
            .iter()
            .take(20)
