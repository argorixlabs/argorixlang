...
enum ScalarType {
    ...
    Integer(String),
    User(String),
    /// `PackageRead` or `BuildWrite`, lent by the driver to `argorix_main`.
    Capability(String),
...
impl ScalarType {
    fn from_ir(value: &CoreIrType) -> Result<Self, CoreCError> {
        match value {
            CoreIrType::Named { name } => match name.as_str() {
                ...
                "PackageRead" | "BuildWrite" => Ok(Self::Capability(name.clone())),
    ...
    fn c_name(&self) -> String {
        match self {
            ...
            Self::Capability(_) => "argorix_capability".into(),
    ...
    fn mangle(&self) -> String {
        match self {
            ...
            Self::Capability(name) => name.to_lowercase(),
...
impl<'a> Emitter<'a> {
    ...
    fn emit(mut self) -> Result<CoreCOutput, CoreCError> {
        ...
        // The entry takes nothing, or the capabilities the driver lends it:
        // at most one `PackageRead` and one `BuildWrite`.
        let mut lent = BTreeSet::new();
        let capabilities_only = entry.parameters.iter().all(|parameter| {
            matches!(parameter, ScalarType::Capability(kind) if lent.insert(kind.clone()))
        });
        if !capabilities_only || !matches!(entry.result, ScalarType::Bool | ScalarType::Integer(_))
        {
            return Err(CoreCError::unsupported(
                "`argorix_main` must return a scalar and take no parameters, or at most one `PackageRead` and one `BuildWrite`",
            ));
        }
        // Only a program that can hold a capability gets the host shim, so
        // every other binary carries no file-system code at all.
        let uses_host = self.signatures.values().any(|signature| {
            signature
                .parameters
                .iter()
                .any(|parameter| matches!(parameter, ScalarType::Capability(_)))
        });

        let mut source = String::new();
        source.push_str("/* generated by Argorix Core C backend 0.1 */\n");
        if uses_host {
            // realpath and O_NOFOLLOW are POSIX, outside strict C11, and macOS
            // hides O_NOFOLLOW unless its extensions are on. The macros must
            // come before the first system header.
            source.push_str(
                "#define _XOPEN_SOURCE 700\n#ifdef __APPLE__\n#define _DARWIN_C_SOURCE 1\n#endif\n",
            );
        }
        source.push_str("#include \"argorix_core_runtime.h\"\n");
        if uses_host {
            source.push_str("#include \"argorix_core_host.h\"\n");
        }
        source.push_str(
            "\
             #include <inttypes.h>\n\
    ...
    fn emit_main(&self, entry: &Signature, source: &mut String) -> Result<(), CoreCError> {
        let kinds = entry
            ...
            .filter_map(|parameter| match parameter {
                ScalarType::Capability(kind) => Some(kind.as_str()),
                _ => None,
            })
            .collect::<Vec<_>>();
        if kinds.is_empty() {
            ...
            // The driver names the roots and budgets on the command line; the
            // shim canonicalizes them before any Core code runs, and a missing
            // one traps PERMISSION_DENIED.
            writeln!(
                source,
                "int main(int argc, char **argv) {{\n    argorix_host_start(argc, argv, {}, {});",
                kinds.contains(&"PackageRead"),
                kinds.contains(&"BuildWrite")
            )
            .unwrap();
        }
        source.push_str("    argorix_budget budget = {ARGORIX_STEP_LIMIT, ARGORIX_DEPTH_LIMIT};\n");
        ...
        writeln!(
            ...
            kinds
                .iter()
                .map(|kind| format!(
                    ", argorix_host_capability({})",
                    u8::from(*kind == "BuildWrite") + 1
                ))
                .collect::<String>()
        )
        .unwrap();
...
impl<'a> FunctionEmitter<'a> {
        ...
        match expression {
            ...
            CoreIrExpr::Call { callee, arguments } => {
                ...
                if let CoreIrExpr::Field { value, name } = callee.as_ref() {
                    if let CoreIrExpr::Path { segments } = value.as_ref() {
                        if let [local] = segments.as_slice() {
                            if let Some(ScalarType::Capability(kind)) =
                                self.locals.get(local).cloned()
                            {
                                return self
                                    .emit_host_call(local, &kind, name, arguments, indent, source);
                            }
                        }
                    }
                    if name == "push" {
    ...
    /// A compiler-host operation on a capability. The views are bound first,
    /// so the host sees each argument evaluated once, in order; the host
    /// header re-checks the capability, the path and the budget.
    fn emit_host_call(
        &mut self,
        receiver: &str,
        kind: &str,
        name: &str,
        arguments: &[CoreIrExpr],
        indent: usize,
        source: &mut String,
    ) -> Result<(String, ScalarType), CoreCError> {
        let bytes = ScalarType::Slice(Box::new(ScalarType::Integer("u8".into())));
        let mut views = Vec::new();
        for argument in arguments {
            let value = self.emit_expr(argument, Some(&bytes), indent, source)?;
            views.push(self.bind_temp(value.0, bytes.clone(), indent, source)?.0);
        }
        let code = ScalarType::Integer("u64".into());
        match (kind, name, views.as_slice()) {
            ("PackageRead", "status", [path]) => self.bind_temp(
                format!(
                    "argorix_host_package_status(argorix_v_{receiver}, {path}.data, {path}.length)"
                ),
                code,
                indent,
                source,
            ),
            ("PackageRead", "read", [path]) => self.bind_temp(
                format!(
                    "argorix_host_package_read(argorix_v_{receiver}, {path}.data, {path}.length, ARGORIX_BUFFER_LIMIT_BYTES)"
                ),
                ScalarType::Buffer(Box::new(ScalarType::Integer("u8".into()))),
                indent,
                source,
            ),
            ("BuildWrite", "write", [path, data]) => self.bind_temp(
                format!(
                    "argorix_host_build_write(argorix_v_{receiver}, {path}.data, {path}.length, {data}.data, {data}.length)"
                ),
                code,
                indent,
                source,
            ...
            _ => Err(CoreCError::unsupported(format!(
                "`{kind}` has no operation `{name}` with {} arguments",
                arguments.len()
            ))),
        }
    }
