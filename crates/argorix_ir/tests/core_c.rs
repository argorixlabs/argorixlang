...
/// Emits an inline Core program with no imports.
#[cfg(unix)]
...
#[test]
fn a_program_without_capabilities_gets_no_host_shim() {
    let source = emit("scalar_success.argx");
    assert!(!source.contains("argorix_core_host.h"), "{source}");
    assert!(source.contains("int main(void)"));
}

/// Symlinks cannot live in the fixture tree (a Windows checkout turns them
/// into files), so the escape cases build their roots here.
#[cfg(unix)]
#[test]
fn the_host_refuses_symlinks_that_leave_their_root_when_cc_is_available() {
    use std::os::unix::fs::symlink;
    let compiler = ["cc", "clang", "gcc"]
        .into_iter()
        .find(|name| Command::new(name).arg("--version").output().is_ok());
    let Some(compiler) = compiler else {
        eprintln!("C compiler unavailable; the host shim is covered by C-enabled CI");
        return;
    };
    // `link/x` goes through a directory symlink, `flink` is a file symlink,
    // and `wlink` in the build root points at a file outside it.
    let source = "core 0.1;
module host.escape;

pub fn argorix_main(package: PackageRead, build: BuildWrite) -> u64 {
    let through_dir: Array<u8, 6> = [108u8, 105u8, 110u8, 107u8, 47u8, 120u8];
    let file_link: Array<u8, 5> = [102u8, 108u8, 105u8, 110u8, 107u8];
    let write_link: Array<u8, 5> = [119u8, 108u8, 105u8, 110u8, 107u8];
    let data: Array<u8, 2> = [111u8, 107u8];
    let a: u64 = package.status(through_dir.as_slice());
    let b: u64 = package.status(file_link.as_slice());
    let c: u64 = build.write(write_link.as_slice(), data.as_slice());
    a * 100u64 + b * 10u64 + c
}
";
    let generated = emit_source(source);
    assert!(generated
        .starts_with("/* generated by Argorix Core C backend 0.1 */\n#define _XOPEN_SOURCE 700\n"));
    let temporary = std::env::temp_dir().join(format!("argorix-host-{}", std::process::id()));
    let _ = fs::remove_dir_all(&temporary);
    let package = temporary.join("package");
    let build = temporary.join("build");
    let outside = temporary.join("outside");
    for directory in [&package, &build, &outside] {
        fs::create_dir_all(directory).unwrap();
    }
    fs::write(outside.join("x"), "secret").unwrap();
    symlink(&outside, package.join("link")).unwrap();
    symlink(outside.join("x"), package.join("flink")).unwrap();
    symlink(outside.join("x"), build.join("wlink")).unwrap();

    let c_file = temporary.join("escape.c");
    let executable = temporary.join("escape");
    fs::write(&c_file, generated).unwrap();
    let compile = Command::new(compiler)
        .args(["-std=c11", "-Wall", "-Wextra", "-Werror", "-pedantic"])
        .arg("-I")
        .arg(root().join("bootstrap/c"))
    ...
    let run = Command::new(&executable)
        .arg("--package-root")
        .arg(&package)
        .args(["--read-budget", "100", "--build-root"])
        .arg(&build)
        .args(["--write-budget", "100"])
        .output()
        .unwrap();
    assert_eq!(
    ...
    assert_eq!(fs::read_to_string(outside.join("x")).unwrap(), "secret");

    // A flag the entry did not ask for is refused before any Core code runs.
    let extra = Command::new(&executable)
        .args(["--bogus", "1"])
        .output()
        .unwrap();
