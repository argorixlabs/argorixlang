...
#[test]
fn resolves_runtime_hardening_package_and_preserves_import_order() {
    let package = resolve_package(&manifest("runtime_hardening_project/argorix.toml")).unwrap();
    let merged = check_package(&package).expect("runtime hardening package checks");
    assert_eq!(merged.runtime_hardening_profiles.len(), 1);
    assert_eq!(merged.threat_models.len(), 1);
    assert_eq!(
        merged.runtime_hardening_profiles[0].name.value,
        "HardenedRuntime"
    );
