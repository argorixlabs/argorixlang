...
The v0.2 controlled matrix is deterministic and local; it is not a randomized
workload, legal analysis, external identity test, physical data-residency
measurement, adversarial campaign, concurrency study, performance benchmark,
or usability study.  The bounded prompt-content field was present in 0/27
complete valid traces; six source-only directories were unassessable.  Thus
prompt-level qualitative analysis and injection measurement are impossible.  We
do not infer attack success from policy violations: those violations report
unknown rules, not successful attacks.
...
\input{tables/limitations-future-assurance}
\end{table}
