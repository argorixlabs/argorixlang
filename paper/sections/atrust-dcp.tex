...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{evidence-chain.pdf}
\caption{Offline evidence verification scope. Source is upstream and outside
bundle verification; verified inputs are bytecode, trace, and security report,
with the ledger digest derived from trace events and matched to the report's
LedgerSummary.}
\label{fig:evidence-chain}
\end{figure}

\Cref{fig:evidence-chain} therefore distinguishes the dashed, unverified source
input from the bundle-verified JSON artifacts.  Its scope does not extend to an
issuer, credential authority, or remote agent.
