...
| --- | --- | --- | --- | --- | --- |
| Claude | ESP-018 — the agent and policy language, migrated (subtasks A–E; A: the lexer) | `claude/agent-lexer-esp018` | IN PROGRESS (A done, pending review) | `compiler/agent_lexer.argx`, `compiler/unicode.argx`, `tests/selfhost/agent/**`, `spec/language/tokens.md`, `tasks/espada/ESP-018.md`, the token dump of `crates/argorix_parser/src/lexer.rs`, `crates/argorixc/tests/agent_lexer_differential.rs`, `crates/argorixc/tests/unicode_tables.rs` | [coordination/claude.md](coordination/claude.md) |
