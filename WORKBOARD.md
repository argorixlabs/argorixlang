...
| --- | --- | --- | --- | --- | --- |
| Claude | ESP-012 — resolution, types and modules in Argorix (subtasks A, B, C) | `claude/check-esp012*` | IN PROGRESS (A merged in #55, B done) | `compiler/check.argx`, `tests/selfhost/check/**`, `spec/core/check.md`, `tasks/espada/ESP-012.md`, the stage0 checker where the port finds a defect, the `argorixc` check dump | [coordination/claude.md](coordination/claude.md) |
